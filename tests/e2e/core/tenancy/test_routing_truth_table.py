...
def test_tui_gateway_model_switch_routing(tmp_path: Path, request: pytest.FixtureRequest) -> None:
    ...
    try:
        ...
        for i, leg in enumerate(legs):
            ...
            if i == 0:
                # The instant derived title precedes the background LLM upgrade.
                # Wait for the scripted provider title before marking the next leg;
                # otherwise its in-flight aux request is attributed to the new host.
                gw.seen_or_wait(
                    lambda m: gw.event("session.title", sid)(m)
                    and (m.get("params", {}).get("payload") or {}).get("title") == "aux-from-main",
                    timeout=120,
                )
